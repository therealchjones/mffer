...
# Test mffer on a Parallels virtual machine
#
printUsage() {
	echo "Usage: $PROGRAMNAME [ vm-name | os-name ]"
	echo ""
	echo "Arguments (choose at most one):"
	echo "  vm-name: name of Parallels virtual machine to use"
	echo "  os-name: name of operating system to use"
	echo ""
	echo "  If no argument is given, will use the first OS of"
	echo "  macOS, Linux, or Windows for which a test-enabled"
	echo "  Parallels virtual machine exists."
}

# Output verbosity. Verbose briefly notes the current step. Debug
# enables output from each program called and shell script tracing with
# set +x
DEBUG="${DEBUG:-}"
VERBOSE="${VERBOSE:-y}"

# Customizable & inherited variables
DEBUGOUT="${DEBUGOUT:-/dev/null}"
VERBOSEOUT="${VERBOSEOUT:-/dev/null}"
MFFER_TEST_ARG="${MFFER_TEST_ARG:-}"
MFFER_TEST_OS="${MFFER_TEST_OS:-}"
...
# Non-customized variables
PYTHON_VERSION="3.10.6"
PROGRAMNAME="$(basename "$0")"
PROGRAMDIR="$(dirname "$0")"

# As convention, only main() should exit the program and should do so only if
# it's unreasonable to continue without the step succeeding.
main() {
	trap cleanup EXIT
	setVerbosity
	checkArgs "$@"
...
checkArgs() {
	if [ "$#" -gt 1 ]; then
		echo "Error: Too many arugments" >&2
		printUsage >&2
		return 1
	elif [ "$#" -eq 1 ]; then
		if [ -z "$1" ]; then
			echo "Warning: Argument is null. Ignoring." >&2
		elif [ -n "$MFFER_TEST_ARG" ]; then
			echo "Warning: overriding MFFER_TEST_ARG ("$MFFER_TEST_ARG") with command line argument ("$1")" >&2
			MFFER_TEST_ARG="$1"
		fi
	fi
}
...
setVm() {
	vm=""
	if [ -z "$MFFER_TEST_ARG" ] && [ -n "$VM_NAME" ]; then
		return 0
	fi
	if [ -n "$MFFER_TEST_ARG" ]; then
		if vm="$(getVmNames | grep -i "^${MFFER_TEST_ARG}$")" 2>"$DEBUGOUT" && [ -n "$vm" ]; then
			if [ -n "$VM_NAME" ]; then
				echo "Warning: argument '$MFFER_TEST_ARG' is overriding VM_NAME '$VM_NAME'" >&2
			fi
			if [ "$(echo "$vm" | wc -l)" -gt 1 ]; then
				echo "Warning: Found multiple possible VMs:" >&2
				echo "$vm" >&2
				vm="$(echo "$vm" | head -n 1)"
				echo "Using '$vm'" >&2
			fi
		...
		# Else treat MFFER_TEST_ARG as an OS name
		argos=""
		case "$MFFER_TEST_ARG" in
			[Mm][Aa][Cc]* | [Oo][Ss][Xx] | [Oo][Ss][\ -][Xx])
				argos=mac
				;;
			[Ww][Ii][Nn]* | [Mm][Ii][Cc][Rr][Oo]*)
				argos=win
				;;
			[Ll][Ii][Nn]* | [Uu][Bb][Uu]*)
				argos=linux
				;;
			*)
				echo "Error: Unable to parse argument '$MFFER_TEST_ARG'" >&2
				printUsage >&2
				return 1
				;;
		esac
		if [ -n "$MFFER_TEST_OS" ]; then
			echo "Warning: argument '$MFFER_TEST_ARG' is overriding MFFER_TEST_OS '$MFFER_TEST_OS'" >&2
		fi
		MFFER_TEST_OS="$argos"
	fi
	if [ -n "$MFFER_TEST_OS" ]; then
