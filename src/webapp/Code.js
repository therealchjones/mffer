...
function activateSection(name) {
  ...
  if (!cell) {
    ...
    ui.alert("Error", "The section " + name + " was not found.", ui
             .ButtonSet.OK)
    return false;
  } else {
...
function rebuildCharacterUniformValidation() {
  ...
  var spreadsheet = SpreadsheetApp.openById(
    "[spreadsheet ID not specified]");
  var characterSheet = getSheetById(spreadsheet,
  1392219691); // the "Characters" sheet
  var workingSheet = getSheetById(spreadsheet,
  1291082607); // the "Characters Working Sheet" sheet
  ...
  var validationFinder = spreadsheet.createTextFinder('Uniform 1')
    .matchCase(true)
  ...
  for (var i = startRow; i < startRow + rows; i++) {
    var validationRange = workingSheet.getRange(i,
      validationStartColumn, 1, 13);
...
function syncFloorData( floor, opp1, opp2, opp3, selected, team1, team2, team3, preferences ) {
  var spreadsheet = SpreadsheetApp.openById(
    "[spreadsheet ID not specified]");
...
function saveShadowlandRecord() {
  ...
  var workingsheet = spreadsheet.getSheetByName(
    'Shadowland Working Sheet');
  var validity = workingsheet.getRange('$S$2');
  if (validity.isBlank()) {
    ui = SpreadsheetApp.getUi();
    ui.alert('Error', 'Select an opposing team and a winning team.',
      ui.ButtonSet.OK);
    return false;
  ...
  var calculator = spreadsheet.getSheetByName(
  ...
  var newentry = [
    timestamp, floorNum, floorMode, , , , opponentTeam, winningTeam
  ];
...
function resetShadowlandFloor() {
  ...
  var calculator = SpreadsheetApp.getActiveSpreadsheet()
    .getSheetByName('Shadowland Calculator');
  calculator.getRangeList(['$D$2', '$G$2', '$J$2', '$D$6', '$G$6',
    '$J$6'
  ]).setValue(null);
  calculator.getRangeList(['$D$4', '$G$4', '$J$4', '$D$7', '$G$7',
    '$J$7', '$F$10'
  ]).uncheck();
}
...
function nextShadowlandFloor() {
  ...
  if (oldfloor >= 30) {
    ...
    var response = ui.alert('Congratulations!',
...
function checkForReset() {
  ...
  var needsRun = workingSheet.getRange(2, 6, resetFrequencies.length)
    .getValues();
