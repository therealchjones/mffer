...
function createNewSpreadsheet(
	...
	let dataSheet = spreadsheet
		.insertSheet("mffer - " + dateString, 1)
		.hideSheet();
	for (let sheet of spreadsheet.getSheets()) {
		if (
			sheet.getSheetId() != coverSheet.getSheetId() &&
			sheet.getSheetId() != dataSheet.getSheetId()
		) {
...
function importNewData(newText: string): void {
	...
	let metadataFinder = spreadsheet
		...
		.withVisibility(SpreadsheetApp.DeveloperMetadataVisibility.DOCUMENT);
	let dataSheet = getSheetById(
		spreadsheet,
		Number(metadataFinder.withKey("mffer-data-sheet").find()[0].getValue())
	);
	dataSheet.copyTo(spreadsheet);
	dataSheet.clear().setName("mffer - " + getDateString());
